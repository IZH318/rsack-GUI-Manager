...
## ⏩ 설치 방법 (초급자용 설명서)
01. Python 공식 홈페이지에서 설치 파일을 다운로드 받거나 zip 파일 다운로드 후 적절한 위치에 압축 해제 합니다. <BR>
...
2. Python 공식 홈페이지에서 다운로드 받은 설치 파일을 실행하거나 `01_Install` 폴더에 있는 설치 파일을 실행하여 설치합니다. <BR>
...
3. 모두 설치가 끝났다면 `키보드 Win + R` 또는 `시작 -> 검색`란에 `cmd`를 입력하여 cmd를 실행합니다. <BR>
...
4. cmd가 실행되었다면 아래 내용을 참고하여 필요한 패키지를 업데이트(선택) 또는 설치 합니다. <BR>
...
만약 위 명령어 사용 중 ERROR: Could not install packages due to an EnvironmentError: [WinError 5] 액세스가 거부되었습니다: (생략) Consider using the `--user` option or check the permissions. 과 같은 오류가 나왔다면 끝에 `--user`를 붙여서 입력 <BR>
